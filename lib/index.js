...
function getName(dir, { navPrefix, stripNumbers } = {}) {
  ...
  //此处直接返回name,不然的话 像TypeScript,SpringCloud本来连在一起的词，被分成了Type Script和Spring Cloud...
  return name;
...
function nav(rootDir, { navPrefix, stripNumbers, skipEmptyNavbar }, relativeDir = "/", currentNavLevel = 1) {
  ...
  if (currentNavLevel > 1 && childrenDirs.length === 0) {
    ...
    // 修复路径错误
	  let temp = relativeDir+sep;
    result = { text: getName(baseDir, { stripNumbers, navPrefix }), link: temp.replace(/\\/g, "/") };
